...
- **Phase 14 — Integration Correctness**: enable/disable enforcement end-to-end (so disabling `claude-code` in config actually prevents claude-code agents from spawning), per-spawn MCP overlays for Cursor/Codex/Copilot (today only Claude Code receives ctx-sys and per-plan MCP servers at run time), Anthropic prompt caching shipped, working OpenAI and OpenRouter provider implementations (today they're stubs that throw at spawn time), a Copilot backend reality check that either fixes the `gh copilot` integration or honestly documents the gap, live backend smoke tests for every integration, Spec Kit parser hardening + `spec.md`/`plan.md` content propagation, a config UX & model-discovery sweep (`plan.agent`/`plan.model`, dead-field cleanup, `merge.history: rebase` default, `yaao agents --models` with per-backend model catalogs, `$schema` URL fix, "exited -1" rendering fix), base-branch auto-detection at init + `--feature-branch` CLI flag plumbed across plan/convert/run (so master-default repos work as smoothly as main-default ones, and the CLI surface matches what the MCP tools already do), and `yaao skills import` — convert claude/cursor/copilot/codex/generic skill formats into yaao skills so any single-provider library becomes cross-provider via MCP. **End-of-phase promise**: any plan step can confidently be assigned to `claude-code`, `cursor`, `codex`, `copilot`, or `api` (with `anthropic` / `openai` / `openrouter`); the config block tells the user honestly what's wired up, what models each backend accepts, what branch the work lands on, and where to look when something breaks; and the user's existing single-provider skill library carries across the matrix without rewrites.
