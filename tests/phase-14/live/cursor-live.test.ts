...
const gate = liveTestGate('cursor', hasBinary('cursor-agent'));

describe.skipIf(!gate.ok)(
  `F14.4 — cursor live smoke (${gate.ok ? 'running' : `skipped: ${gate.reason}`})`,
  () => {
    it('spawns cursor-agent against a trivial prompt and exits cleanly', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'yaao-live-cursor-'));
