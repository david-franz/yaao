...
This phase closes a missing half of the skill lifecycle: yaao already makes a written skill *portable*. Phase 17 makes the act of *writing* the skill cheap by lifting it out of a representative session you already had.
...
| --- | --- | --- |
| **F17.1** | `yaao-distiller` built-in skill (prompt + metadata) | [F17.1-distiller-skill.md](F17.1-distiller-skill.md) |
| **F17.2** | In-session capture (structured self-summary contract, redaction) | [F17.2-session-readers.md](F17.2-session-readers.md) |
| **F17.3** | Skill emission, validation, and post-emit `skills install` | [F17.3-skill-emission.md](F17.3-skill-emission.md) |
| **F17.4** | `yaao_distill` MCP tool (sole entry point) | [F17.4-distill-mcp-tool.md](F17.4-distill-mcp-tool.md) |
| **F17.5** | Skill refinement (re-distill with new session, diff review) | [F17.5-skill-refinement.md](F17.5-skill-refinement.md) |
...
F17.1, F17.2, and F17.3 are tightly coupled and should land as a single PR, not three:

- F17.1's prompt has to know what shape of `SessionRecord` to expect (defined in F17.2).
- F17.1's emitted output has to pass `validateSkill` as F17.3 calls it; if the prompt is wrong the validator rejects.
- F17.3's idempotency guarantees (atomic write, refuse-overwrite-without-merge) only matter if F17.1 is actually producing files.

Sequence:

1. **F17.1 + F17.2 + F17.3** together — `SessionRecord` contract, distiller prompt, emission pipeline. End state: a hand-built `SessionRecord` fed into `runPlanner`-style harness produces a written, validated skill on disk.
2. **F17.4** — the MCP tool wires everything together end-to-end and surfaces it to calling agents. F12.6 hot reload (already shipped) makes the new tool callable in the same session.
3. **F17.5** last — refinement is the multiplier, but only meaningful once F17.1–F17.4 have produced a few real skills.
...
- **A `yaao distill` CLI command.** Distillation is in-context only — see Key Deliverables. Skill *management* commands (list, view, remove) may live under `yaao skills` later, but those are Phase 8 surface, not Phase 17.
- **Running the distilled skill server-side.** Same model as F12.5 — yaao produces the skill; the calling agent uses it.
- **Cross-session synthesis** (combining N unrelated sessions into one skill). Out of scope for v1; refinement (F17.5) handles the iterative case.
